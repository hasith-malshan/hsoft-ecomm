...
if (isset($_GET["s"])) {
    ...
    if (!empty($text)) {
        ...
        $usersrs = Database::s("SELECT * FROM `user` WHERE `email` LIKE '%" . $text . "%' ");
        $d = $usersrs->num_rows;
        $row = $usersrs->fetch_assoc();
        $result_per_page = 20;
        $number_of_pages = ceil($d / $result_per_page);
        $page_first_result = ((int)$pageno - 1) * $result_per_page;
        $selectedrs = Database::s("SELECT * FROM `user` WHERE `email` LIKE '%" . $text . "%' LIMIT " . $result_per_page . " OFFSET " . $page_first_result . " ");
...
?>
            ...
            <?php
            $profileimg = Database::s("SELECT * FROM `profile_img` WHERE `user_email`='" . $srow["email"] . "' ");
        ...
        $number_of_pages = ceil($d / $result_per_page);
        $page_first_result = ((int)$pageno - 1) * $result_per_page;
        $selectedrs = Database::s("SELECT * FROM `user` LIMIT " . $result_per_page . " OFFSET " . $page_first_result . " ");
        $srn = $selectedrs->num_rows;
